      ...
      <Box sx={style}>
        <Container>
          <Typography variant="h4" textAlign="center" paddingBottom={2} paddingTop={1}>
            {isUpdateForm ? <span>Update</span> : <span>Tambah</span>} genre
          </Typography>
          <Stack spacing={3} paddingY={2}>

            <TextField name="name" label="Nama Genre" value={genre.name} autoFocus required
                       onChange={(e) => setGenre({...genre, name: e.target.value})}/>
            <TextField name="description" label="Deskripsi" value={genre.description} multiline
                       rows={2}
                       maxRows={4}
