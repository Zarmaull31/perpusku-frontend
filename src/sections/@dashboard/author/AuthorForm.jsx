      ...
      <Box sx={style}>
        <Container>
          <Typography variant="h4" textAlign="center" paddingBottom={2} paddingTop={1}>
            {isUpdateForm ? <span>Update</span> : <span>Tambah</span>} Penerbit
          </Typography>
          <Stack spacing={3} paddingY={2}>

            <TextField name="name" label="Nama Penerbit" value={author.name} autoFocus required
                       onChange={(e) => setAuthor({
                         ...author,
                         name: e.target.value,
                         photoUrl: `https://avatars.dicebear.com/api/initials/${e.target.value.replace(" ", "+")}.svg`
                       })}/>
            <TextField name="description" label="Deskripsi" value={author.description} multiline
                       rows={2}
                       maxRows={4}
